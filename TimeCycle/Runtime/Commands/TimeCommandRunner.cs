...
namespace FakeMG.TimeCycle
{
    ...
    internal sealed class TimeCommandRunner
    {
        ...
        public TimeCommandFrame Tick(float deltaTimeSeconds)
        {
            ...
            float linearProgress01 = Math.Clamp(_activeCommand.ElapsedSeconds / _activeCommand.DurationSeconds, 0f, 1f);
            float evaluatedProgress01 = Math.Clamp(_activeCommand.TransitionCurve.Evaluate(linearProgress01), 0f, 1f);
            float directedProgress01 = Math.Max(_activeCommand.DirectedProgress01, evaluatedProgress01);
            if (linearProgress01 >= 1f)
            {
                directedProgress01 = 1f;
            }

            double distanceDeltaSeconds = _activeCommand.DirectedDistanceSeconds * (directedProgress01 - _activeCommand.DirectedProgress01);
            _activeCommand.DirectedProgress01 = directedProgress01;
            return new TimeCommandFrame(distanceDeltaSeconds * (int)_activeCommand.Direction, directedProgress01, linearProgress01 >= 1f);
        }
