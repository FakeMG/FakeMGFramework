...
namespace FakeMG.TimeCycle
{
    ...
    /// </summary>
    public sealed class TimeOfCycleService : ITimeOfCycle, ITimeControlContext, IInitializable, ITickable, IDisposable
    {
        ...
        public TimeOfCycleService(TimeOfCycleProfileSO sharedProfileSO, IEnumerable<ITimeOfCycleOutputApplicator> outputApplicators)
        {
            ...
            _commandCoordinator = new TimeCommandCoordinator(_clock, PublishClockNotifications, PublishCommandCompletion);
            _configurationSession = new TimeOfCycleConfigurationSession(sharedProfileSO, _clock, _commandCoordinator, _outputCoordinator);
        }
        ...
        public void Initialize()
        {
            ...
            ResolvedTimeOfCycleConfiguration configuration = _configurationSession.CurrentConfiguration;
            _advancementRateCycleSecondsPerRealSecond = configuration.DefaultAdvancementRateCycleSecondsPerRealSecond;
        ...
        public bool TrySetAdvancementRate(double advancementRateCycleSecondsPerRealSecond)
        {
            ...
            {
                Echo.Warning("Time-of-cycle advancement rate must be a finite, non-negative number. " + $"Received {advancementRateCycleSecondsPerRealSecond}.");
                return false;
            }
        ...
        {
            return TrySetOverride(_configurationSession.TrySetContextOverride, contextOverrideSO);
        }

        public bool SetRuntimeOverride(TimeOfCycleOverrideSO runtimeOverrideSO)
        {
            return TrySetOverride(_configurationSession.TrySetRuntimeOverride, runtimeOverrideSO);
        }
        ...
        private void TickPersistentControlOrAutomaticAdvancement(float deltaTimeSeconds)
        {
            ...
            if (activeRequest == null)
            {
                ...
                {
                    TryAdvanceAuthoritativeTime(_advancementRateCycleSecondsPerRealSecond * deltaTimeSeconds);
                }
        ...
        // TimeOfCycleService cannot call this explicit interface implementation directly.
        // The private method below lets TimeOfCycleService call the logic directly.
        ...
        {
            if (!_clock.TryAdvanceAuthoritativeTime(signedDeltaSeconds, out IReadOnlyList<CycleClockNotification> notifications, out string errorMessage))
            {
        ...
        private bool TrySetOverride(TryApplyOverride tryApplyOverride, TimeOfCycleOverrideSO overrideSO)
        {
            if (!_isInitialized)
            {
        ...
        {
            if (_commandCoordinator.IsActive && _controlArbiter.ActiveRequest != null && !_controlArbiter.ActiveRequest.AllowsTimeCommands)
            {
                _commandCoordinator.Cancel();
            }
        ...
        {
            for (int notificationIndex = 0; notificationIndex < notifications.Count; notificationIndex++)
            {
        ...
        private void UpdatePublicState()
        {
            ...
            if (!_commandCoordinator.IsActive)
            {
                ...
                {
                    isAutomaticAdvancementActive = _controlArbiter.ActiveRequest.Strategy.IsAutomaticAdvancementActive;
                    effectiveRateCycleSecondsPerRealSecond = _controlArbiter.ActiveRequest.Strategy.EffectiveRateCycleSecondsPerRealSecond;
                }
                else if (_isAutomaticAdvancementEnabled)
                {
                    isAutomaticAdvancementActive = true;
                    effectiveRateCycleSecondsPerRealSecond = _advancementRateCycleSecondsPerRealSecond;
                }
            }

            ResolvedTimeOfCycleConfiguration configuration = _configurationSession.CurrentConfiguration;
            double normalizedCycleProgress01 = configuration == null ? 0d : _clock.AuthoritativeTimeSeconds / configuration.CycleDurationSeconds;
        ...
        private delegate bool TryApplyOverride(TimeOfCycleOverrideSO overrideSO, out CyclePeriodChange? periodChange, out string errorMessage);
    }
}
