    ...
    public sealed class AmbientRenderSettingsOutputApplicator : MonoBehaviour, ITimeOfCycleOutputApplicator
    {
        ...
        private void ApplyMode(IReadOnlyCycleOutputState outputState)
        {
            ...
            if (ambientMode == AmbientMode.Custom)
            {
                ...
                {
                    Echo.Warning("Custom spherical-harmonics ambient mode is not supported by the day-night applicator.", context: this);
            ...
            if (ambientMode != AmbientMode.Skybox && ambientMode != AmbientMode.Trilight && ambientMode != AmbientMode.Flat)
            {
