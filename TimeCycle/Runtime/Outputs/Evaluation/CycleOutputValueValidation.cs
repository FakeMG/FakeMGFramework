...
namespace FakeMG.TimeCycle
{
    ...
    internal static class CycleOutputValueValidation
    {
        ...
        public static bool TryValidate(IReadOnlyList<FloatCyclePoint> points, out string errorMessage)
        {
            for (int pointIndex = 0; pointIndex < points.Count; pointIndex++)
            {
                if (points[pointIndex] != null && CycleNumericValidation.IsFinite(points[pointIndex].Value))
                {
                    continue;
                }
        ...
        public static bool TryValidate(IReadOnlyList<ColorCyclePoint> points, out string errorMessage)
        {
            for (int pointIndex = 0; pointIndex < points.Count; pointIndex++)
            {
        ...
        public static bool TryValidate(IReadOnlyList<RotationCyclePoint> points, out string errorMessage)
        {
            for (int pointIndex = 0; pointIndex < points.Count; pointIndex++)
            {
        ...
        public static bool IsValidRuntimeValue(object value, Type expectedType)
        {
            ...
            {
                return IsFinite(colorValue.r) && IsFinite(colorValue.g) && IsFinite(colorValue.b) && IsFinite(colorValue.a);
            }

            if (value is Quaternion rotationValue)
            {
                return IsFinite(rotationValue.x) && IsFinite(rotationValue.y) && IsFinite(rotationValue.z) && IsFinite(rotationValue.w);
            }
