...
namespace FakeMG.TimeCycle
{
    ...
    internal sealed class ContinuousCycleOutputEvaluator<T> : ICycleOutputEvaluator
    {
        ...
        public object Evaluate(double presentationTimeSeconds)
        {
            ...
            float segmentProgress01 = segmentDurationSeconds <= 0d ? 1f : (float)((evaluatedTimeSeconds - previousTimeSeconds) / segmentDurationSeconds);
        ...
        private static int CompareValuesByTime(ContinuousCycleValue<T> left, ContinuousCycleValue<T> right)
        {
