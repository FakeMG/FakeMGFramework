...
namespace FakeMG.TimeCycle
{
    ...
    internal sealed class CycleOutputCoordinator
    {
        ...
        public bool TryValidateContracts(ResolvedTimeOfCycleConfiguration configuration, out string errorMessage)
        {
        ...
        private bool ValidateContracts(ResolvedTimeOfCycleConfiguration configuration, out string errorMessage)
        {
            ...
            for (int definitionIndex = 0; definitionIndex < configuration.OutputDefinitions.Count; definitionIndex++)
            {
                ...
                if (!CycleOutputValueValidation.IsValidRuntimeValue(initialValue, definition.OutputKeySO.ValueType))
                {
                    errorMessage = $"Evaluator for '{definition.OutputKeySO.name}' produced an invalid initial value.";
                    return false;
                }
