...
namespace FakeMG.SOEventSystem.Editor
{
    public class EventChannelInspectorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (_selectedChannel)
            {
                Rect detailsRect = new Rect(0, topBarHeight + _treeViewHeight + SPLITTER_WIDTH, position.width,
                    totalHeight - _treeViewHeight - SPLITTER_WIDTH);
                _scrollPosition = GUI.BeginScrollView(detailsRect, _scrollPosition,
                    new Rect(0, 0, position.width - 20, 1000));
                    ...
                    while (iterator.NextVisible(enterChildren))
                    {
                        if (iterator.propertyType == SerializedPropertyType.Generic && iterator.type.Contains("Event"))
                        {
                            SerializedProperty persistentCalls =
                                iterator.FindPropertyRelative("m_PersistentCalls.m_Calls");
                                ...
                                for (int i = 0; i < persistentCalls.arraySize; i++)
                                {
                                    ...
                                    if (targetProp != null && targetProp.objectReferenceValue && methodNameProp != null)
                                    {
                                        ...
                                        if (targetMono)
                                        {
                                            ...
                                            var targetFields = targetType.GetFields(BindingFlags.Public |
