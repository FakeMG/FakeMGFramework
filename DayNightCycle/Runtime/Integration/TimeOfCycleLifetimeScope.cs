...
namespace FakeMG.DayNightCycle
{
    ...
    public sealed class TimeOfCycleLifetimeScope : LifetimeScope
    {
...
#if UNITY_EDITOR
        public void ConfigureForEditor(TimeOfCycleProfileSO sharedProfileSO, IEnumerable<MonoBehaviour> outputApplicatorBehaviours)
        {
        ...
        {
            List<ITimeOfCycleOutputApplicator> outputApplicators = new(_outputApplicatorBehaviours.Count);
            for (int behaviourIndex = 0; behaviourIndex < _outputApplicatorBehaviours.Count; behaviourIndex++)
            {
