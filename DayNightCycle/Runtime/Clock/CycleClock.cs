...
namespace FakeMG.DayNightCycle
{
    ...
    internal sealed class CycleClock
    {
        ...
        public bool TryAdvanceAuthoritativeTime(double signedDeltaSeconds, out IReadOnlyList<CycleClockNotification> notifications, out string errorMessage)
        {
            ...
            {
                errorMessage =
                    $"One update may cross at most {maximumCycleCrossings} full cycles " + $"({MAX_BOUNDARY_NOTIFICATION_COUNT} boundary notifications).";
                return false;
            }
        ...
        public bool SetAuthoritativeDestination(double destinationTimeSeconds, out CyclePeriodChange periodChange)
        {
        ...
        public double GetDirectedDistanceSeconds(double startTimeSeconds, double targetTimeSeconds, TimeMovementDirection direction)
        {
        ...
        private List<CycleBoundary> CreateBoundaries(double startTimeSeconds, double signedDeltaSeconds)
        {
            ...
            double firstWrapDistanceSeconds = isForward ? cycleDurationSeconds - startTimeSeconds : startTimeSeconds;
        ...
        private void AddPeriodChangeIfDifferent(CyclePeriodId destinationPeriodId, ICollection<CycleClockNotification> notifications)
        {
            if (CurrentPeriodId.Equals(destinationPeriodId))
            {
                return;
        ...
        private readonly struct CycleBoundary
        {
            ...
            private CycleBoundary(double distanceSeconds, bool isCycleCompletion, CyclePeriodId destinationPeriodId)
            {
            ...
            public static CycleBoundary PeriodBoundary(double distanceSeconds, CyclePeriodId destinationPeriodId)
            {
