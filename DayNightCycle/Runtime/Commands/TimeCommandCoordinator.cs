...
namespace FakeMG.DayNightCycle
{
    ...
    internal sealed class TimeCommandCoordinator
    {
            ...
            out bool hasChangedPresentationImmediately)
        {
            ...
            if (!CanExecute(commandId, command, isServiceInitialized, areCommandsAllowed, cancellationToken, out TimeCommandStatus rejectionStatus))
            {
            ...
            ResolveTransition(command.Transition, out float transitionDurationSeconds, out AnimationCurve transitionCurve);
            double startTimeSeconds = command.Mode == TimeCommandMode.SimulatedAdvance ? _clock.AuthoritativeTimeSeconds : _clock.PresentationTimeSeconds;
            double directedDistanceSeconds = _clock.GetDirectedDistanceSeconds(startTimeSeconds, targetTimeSeconds, command.Direction);
            ...
            {
                CompleteZeroDurationCommand(command.Mode, command.Direction, targetTimeSeconds, directedDistanceSeconds);
                hasChangedPresentationImmediately = true;
                return CompleteImmediate(commandId, TimeCommandStatus.Completed);
            }
        ...
        private void ResolveTransition(TimeCommandTransition requestedTransition, out float durationSeconds, out AnimationCurve transitionCurve)
        {
            ...
            transitionCurve = requestedTransition.EasingCurve == null ? _configuration.DefaultCommandTransitionCurve : requestedTransition.EasingCurve;
        }
