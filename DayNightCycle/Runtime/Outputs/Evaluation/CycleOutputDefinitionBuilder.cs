    ...
    internal static class CycleOutputDefinitionBuilder
    {
            ...
            where TPoint : IContinuousCyclePoint<TValue>
        {
            ...
            for (int pointIndex = 0; pointIndex < points.Count; pointIndex++)
            {
                ...
                values.Add(new ContinuousCycleValue<TValue>(point.ResolveTimeSeconds(cycleDurationSeconds), point.Value));
            }

            return new ContinuousCycleOutputEvaluator<TValue>(cycleDurationSeconds, values, interpolationCurve, interpolate);
        }
        ...
        public static ICycleOutputEvaluator CreateDiscrete<TPoint, TPeriodValue, TValue>(
            ...
            IReadOnlyList<ResolvedCyclePeriod> periods)
            ...
            where TPeriodValue : IPeriodCycleValue<TValue>
        {
            ...
            for (int pointIndex = 0; pointIndex < timelinePoints.Count; pointIndex++)
            {
                ...
                changes.Add(new DiscreteCycleChange<TValue>(point.ResolveTimeSeconds(cycleDurationSeconds), 1, point.Value));
            }
