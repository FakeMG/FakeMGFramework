...
namespace FakeMG.DayNightCycle
{
    ...
    internal sealed class CycleOutputRuntimeSet
    {
        ...
        public void Configure(ResolvedTimeOfCycleConfiguration configuration, double presentationTimeSeconds, bool isInitialConfiguration)
        {
            Dictionary<CycleOutputKeySO, CycleOutputRuntimeEntry> replacementEntries = new();
            for (int definitionIndex = 0; definitionIndex < configuration.OutputDefinitions.Count; definitionIndex++)
            {
                CycleOutputDefinition definition = configuration.OutputDefinitions[definitionIndex];
                ICycleOutputEvaluator evaluator = definition.CreateEvaluator(configuration.CycleDurationSeconds, configuration.Periods);
                object destinationValue = evaluator.Evaluate(presentationTimeSeconds);
                if (!CycleOutputValueValidation.IsValidRuntimeValue(destinationValue, definition.OutputKeySO.ValueType))
                {
                    Echo.Error(
                    ...
                    destinationValue = CycleOutputValueValidation.GetDefaultValue(definition.OutputKeySO.ValueType);
                }

                _entryByOutputKey.TryGetValue(definition.OutputKeySO, out CycleOutputRuntimeEntry previousEntry);
                ...
                CycleOutputRuntimeEntry replacementEntry = new(definition, evaluator, currentValue, currentValue, canTransition);
        ...
        public void Update(double presentationTimeSeconds, float deltaTimeSeconds)
        {
            foreach (CycleOutputRuntimeEntry entry in _entryByOutputKey.Values)
            {
                ...
                if (!CycleOutputValueValidation.IsValidRuntimeValue(destinationValue, entry.Definition.OutputKeySO.ValueType))
                {
                ...
                float transitionProgress01 = transitionDurationSeconds <= 0f ? 1f : Math.Clamp(entry.TransitionElapsedSeconds / transitionDurationSeconds, 0f, 1f);
                entry.CurrentValue = entry.Definition.InterpolateProfileValue(entry.TransitionStartValue, destinationValue, transitionProgress01);
                if (!CycleOutputValueValidation.IsValidRuntimeValue(entry.CurrentValue, entry.Definition.OutputKeySO.ValueType))
                {
