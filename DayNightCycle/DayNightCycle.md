...
* Profiles author timeline positions as normalized progress in `[0, 1)` so changing cycle length preserves the timeline shape.
* Runtime state, commands, and the cycle clock use seconds.
...
* A period is defined by an identifier and normalized start progress.
...
* Continuous outputs define configurable normalized-progress points along the cycle.
...
* Normalized timeline points and values.
