...
namespace FakeMG.DayNightCycle.Editor
{
    ...
    public static class DayNightCycleLegacyAssetMigration
    {
        ...
        private const string OUTPUT_KEY_ROOT_PATH = "Assets/Thirdparty/FakeMGFramework/DayNightCycle/Output Keys";
        ...
        public static void MigrateAllAssets()
        {
            ...
            Echo.Log($"Day-night migration version {CURRENT_MIGRATION_VERSION} completed. " + $"Updated {migratedAssetCount} assets.");
        }

        [MenuItem(FakeMGEditorMenus.DAY_NIGHT_CYCLE + "/Migrate Timeline Positions")]
        public static void MigrateTimelinePositions()
        {
            int migratedAssetCount = MigrateTimelineAssets<TimeOfCycleProfileSO>();
            migratedAssetCount += MigrateTimelineAssets<TimeOfCycleOverrideSO>();
            AssetDatabase.SaveAssets();
            Echo.Log($"Normalized day-night timeline migration updated {migratedAssetCount} assets.");
        }
